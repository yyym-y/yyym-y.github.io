...
xlsColumnsMapper = {"id" : 0, "keyId" : 1, "type" : 2, "projectName": 3, "headAuthor" : 6}
...
def formName( keyId, name, headAuthor ):
    return keyId + "+" + name[ : 5] + "+" + headAuthor
    ...
    if pos != -1:
        ...
        if len(projectName) > 5 :
            ...
            if projectName in xlsInfosMapByProjectName_5:
                return formName( xlsInfosMapByProjectName_5[projectName].keyId , 
        ...
        if headAuthor in xlsInfosMapByProjectAuthor:
            return formName( xlsInfosMapByProjectAuthor[headAuthor].keyId, 
...
def getStanderdName( ori_name ):
    fileType = ori_name[ ori_name.rfind(".") :]
    # try to find KeyId
    pos = ori_name.find( "RJ" )
    if pos != -1 :
        keyId = ori_name[pos - 2 : pos + 6]
        if keyId in xlsInfosMapByKeyId:
            return formName( keyId, xlsInfosMapByKeyId[keyId].projectName, 
                                xlsInfosMapByKeyId[keyId].headAuthor ) + fileType
